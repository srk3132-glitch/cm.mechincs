...
export function Card({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-2xl border border-slate-200/80 bg-white/90 shadow-sm shadow-slate-200/50 backdrop-blur",
        "transition-colors duration-300 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-black/20",
        className,
      )}
      {...props}
    >
...
export function Button({
  ...
  const variants: Record<Variant, string> = {
    primary:
      "bg-brand-600 text-white hover:bg-brand-700 shadow-sm shadow-brand-600/30 dark:bg-brand-500 dark:hover:bg-brand-400",
    secondary:
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      className={cn(
        "inline-flex rounded-lg bg-slate-100 p-0.5 text-xs font-medium dark:bg-slate-800",
        className,
      )}
      role="tablist"
    >
...
export function StatTile({
  ...
  return (
    <div className="rounded-xl border border-slate-200/70 bg-slate-50/70 px-3.5 py-3 dark:border-slate-800 dark:bg-slate-950/40">
